...
public static class ServiceProxy
{
    ...
    {
        var baseUrl = configuration[$"Services:{serviceKey}:BaseUrl"]
            ?? throw new InvalidOperationException($"Services:{serviceKey}:BaseUrl is required.");
        var targetUri = BuildTargetUri(baseUrl, targetSegment, path, context.Request.QueryString);
    ...
    private static Uri BuildTargetUri(string baseUrl, string targetSegment, string? path, QueryString queryString)
    {
        var normalizedBaseUrl = baseUrl.TrimEnd('/') + "/";
        var normalizedPath = string.IsNullOrWhiteSpace(path) ? string.Empty : "/" + path.TrimStart('/');
        return new Uri(new Uri(normalizedBaseUrl), $"api/v1/{targetSegment.Trim('/')}{normalizedPath}{queryString}");
    }
